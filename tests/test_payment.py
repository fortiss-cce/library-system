...
def credit_card_high_limit(user: User, card: CreditCard, invoice: Invoice):
    ...
    card.amount = invoice.calculate_fee()[0] + 1.0
...
def account_balance_changed(paypal, invoice: Invoice):
    user.reading_credits = 0
    assert PAYPAL_ACCOUNT_BALANCE[paypal[0]] == 100.0 - invoice.calculate_fee()[0]
